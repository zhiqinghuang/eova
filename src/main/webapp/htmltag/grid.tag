...
<script>
	$(function(){
			...
			columns:[[
				...
				<%for(item in items){%>
					<%if(item.isShow!true){%>
						...
						<%}%>
                        <%
                        // Grid Cell Editor,自增长禁止编辑
                        if(isTrue(obj.isCellEdit)){
                            if(!item.isAuto && item.isEdit){
                                print("editor:'" + item.editor! + "',");
                            }
                        }
                        %>
						},
        ...
        // 开启编辑模式
        myGrid.datagrid().datagrid('enableCellEditing');
