		...
		<%for(item in items){%>
			...
			<div class="lbl">${item.cn}:</div>
				...
				<%}else if(item.type == "查找框"){%>
					<div>
						<div id="${item.en}" name="${QUERY}${item.en}" value="${item.value!}" code="${item.objectCode}" class="eova-find"
				...
				<%}else {%>
					<%// 默认为文本框%>
					<div>
						<div id="${item.en}" name="${QUERY}${item.en}" value="${item.value!}" class="eova-text"></div>
					</div>
				<%}%>
			</div>
		<%}%>
